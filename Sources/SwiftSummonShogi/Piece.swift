...
private extension Piece {
    nonisolated(unsafe) static let pieceAttacks: [Self: Set<Attack>] = Dictionary(uniqueKeysWithValues: piecesAndAttacks)
...
extension Piece.Kind: CaseIterable {
    nonisolated(unsafe) public static let allCases: [Self] = [
...
extension Piece: CaseIterable {
    nonisolated(unsafe) public static let allCases: [Self] = kindsAndColors.map(Self.init)
